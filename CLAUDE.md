...
| `memory_logs.md` | セッション間の長期記憶（マイルストーン・合意事項） |
| `直前対策チェックリスト.md` | 試験前日チェックリスト |
...
**学習セッション開始時の必須手順**:
1. `learning/memory_logs.md`（文脈・マイルストーン）を読む
2. `learning/学習計画書_資格同時取得.md`（全体進捗）を読む
3. `learning/進捗ログ.md`（ダッシュボード）を読む
4. `learning/学習記録/` の最新数日分を読む

**学習記録とmemory_logsの役割分担**:
- `学習記録/`: セッション内の詳細ログ（問題ごとの正誤、つまずき詳細）
- `memory_logs.md`: セッション横断の長期記憶（方針変更、合意事項、心理状態の変化）
