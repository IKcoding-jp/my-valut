...
name: retrospective
description: >-
  FE学習特化の週次レトロスペクティブ＆学習戦略の自動最適化。
  「レトロスペクティブ」「戦略見直し」「学習の振り返り」「今週どうだった？」「成長してる？」
  「進捗分析」等で起動。過去1週間の学習記録・ダッシュボード・memory_logsを横断分析し、
  「伸びた分野」「停滞分野」「新弱点」を特定。学習計画の修正提案を生成する。
  注意: 生活全般のレビュー（習慣・キャリア・ジャーナル集計）には review スキルを使用すること。
---
...
読み込むファイル:
1. `learning/基本情報技術者/進捗ログ.md` — 現在のダッシュボード数値
2. `学習記録/` — 対象期間の全日次記録
3. `memory_logs.md` — 対象期間のマイルストーン・合意事項
4. `learning/基本情報技術者/学習計画表.md` — 週別スケジュール（計画 vs 実績の比較用）
...
- このスキルは **memory_logs.md と学習記録への追記のみ** 行う（learning/基本情報技術者/進捗ログ.md は更新しない）
- learning/基本情報技術者/学習計画表.md への直接編集は **ユーザーの合意を得てから** 行う
- 分析結果は客観的データに基づく（主観的な評価は避ける）
- CLAUDE.mdの「回答のルール（FE学習時）」を適用
