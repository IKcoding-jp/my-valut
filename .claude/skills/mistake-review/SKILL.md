...
1. `learning_progress.md` — 危険問題リスト・概念混同リスト・間違いパターン集計
...
`learning_progress.md` の「間違いパターン集計」テーブルと、
...
2. `learning_progress.md` の間違いパターン集計テーブルを参照
3. 概念混同リストから混同ペアを抽出
...
既存の概念混同リスト（`learning_progress.md`）に加え、以下の条件で**新たな混同候補**を自動検出:
...
ユーザーが「はい」と答えた場合、`learning_progress.md` の概念混同リストへの追加を提案する
...
- このスキルは基本的にファイルの更新を行わない（読み取り専用）
  - 例外: ユーザーが概念混同リストへの追加を承認した場合のみ、`learning_progress.md` を更新
